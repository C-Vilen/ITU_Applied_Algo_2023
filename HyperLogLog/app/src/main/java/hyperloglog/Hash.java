...
/**
 * This file provides our implementation of the hash function for testing in
 * codeGrade.
...
public class Hash {
    ...
    public static int computeHash(int x) {
        int[] A = getMatrix(); // Initiate Matrix
    ...
    public static String formatForCodeGrade(int x) {
        String hexString = String.format("%08X", x).toLowerCase();

        return hexString;
    }

    /**
     * Homemade non-cryptographic random has function based on FNV-1 but with added prime constant for fewer collisions.
     * Used for early testing of all the other functions.
     */
    public static int rumleHash(int y) {
        
        var toHash = BigInteger.valueOf(y).toByteArray();
        int INIT = 0x811c9dc5;
    ...
    public static void main(String[] args) {
        ...
        while (in.hasNext()) {
            ...
            int hashValue = computeHash((int) x);
            System.out.println(formatForCodeGrade(hashValue));
        }
        in.close();
    }
}
